{{-- @php
    ...
    $name = $name ?? '';
    $value = $value ?? []; // list pilihan dropdown
    $selected = $selected ?? old($name); // nilai terpilih, fallback ke old()
@endphp
                ...
                @if (is_array($item) && isset($item['label']) && isset($item['children']))
                    <optgroup label="{{ $item['label'] }}">
                        @foreach ($item['children'] as $child)
                            <option value="{{ $child }}" {{ $selected == $child ? 'selected' : '' }}>
                                {{ $child }}
                            </option>
                        @endforeach
                    </optgroup>
                    ...
                    <optgroup label="{{ $item['label'] }}">
                        @foreach ($item['children'] as $child)
                            @if (is_array($child) && isset($child['value'], $child['display']))
                                <option value="{{ $child['value'] }}" {{ $selected == $child['value'] ? 'selected' : '' }}>
                                    {{ $child['display'] }}
                                </option>
                            @endif
                        @endforeach
