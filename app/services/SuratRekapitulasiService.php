...
class SuratRekapitulasiService
{
    public function getChartData($query, $groupBy = 'daily')
    {
        ...
        switch ($groupBy) {
            case 'monthly':
                $data = $query->selectRaw("DATE_FORMAT(tanggal_terima, '%Y-%m') as label, klasifikasi_surat, COUNT(*) as total")
                    ->groupBy('label', 'klasifikasi_surat')
                    ->orderBy('label')
                    ->get();

                $labelList = $data->pluck('label')->unique()->sort()->values();
                $categories = $labelList->map(fn($val) => \Carbon\Carbon::parse($val . '-01')->translatedFormat('F Y'))->toArray();
                break;

            case 'weekly':
                $data = $query->selectRaw("YEAR(tanggal_terima) as tahun, WEEK(tanggal_terima, 1) as minggu, klasifikasi_surat, COUNT(*) as total")
            ...
            default: // daily
                $data = $query->selectRaw("DATE(tanggal_terima) as label, klasifikasi_surat, COUNT(*) as total")
                    ->groupBy('label', 'klasifikasi_surat')
                    ->orderBy('label')
                    ->get();

                $labelList = $data->pluck('label')->unique()->sort()->values();
                $categories = $labelList->map(fn($val) => \Carbon\Carbon::parse($val)->translatedFormat('d M'))->toArray();
                break;
        }
